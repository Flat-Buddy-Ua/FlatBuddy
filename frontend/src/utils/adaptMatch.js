...
const ROOM_SHARING_LABELS = {
    get 1() { return i18n.t('card.room_sharing_1', 'Комфортно ділити кімнату'); },
    get 2() { return i18n.t('card.room_sharing_2', 'Хочу окрему кімнату'); },
};
const PREFERRED_GENDER_LABELS = {
    get 1() { return i18n.t('card.gender_1', 'Лише з хлопцями'); },
    get 2() { return i18n.t('card.gender_2', 'Лише з дівчатами'); },
    get 3() { return i18n.t('card.gender_3', 'Не має значення'); },
};
const HOUSING_STATUS_LABELS = {
    get 1() { return i18n.t('card.housing_status_1', 'Шукає квартиру та сусіда'); },
    get 2() { return i18n.t('card.housing_status_2', 'Є квартира, шукає сусіда'); },
};
...
function econLabel(value) {
    if (value === null || value === undefined) return i18n.t('card.unknown', 'Невідомо');
    if (value <= -4) return i18n.t('card.left', 'Лівий');
    if (value >= 4) return i18n.t('card.right', 'Правий');
    return i18n.t('card.centrist', 'Центрист');
}

/** Підпис для соціальної осі */
function socLabel(value) {
    if (value === null || value === undefined) return i18n.t('card.unknown', 'Невідомо');
    if (value <= -4) return i18n.t('card.liberal', 'Ліберальний');
    if (value >= 4) return i18n.t('card.communitarian', 'Комунітарний');
    return i18n.t('card.centrist', 'Центрист');
}
...
export function adaptMatch(match) {
    ...
    const housingTable = [
        ...
        ['Тваринка', housing.has_pet
            ? (housing.pet_description || i18n.t('card.pet_yes', 'Є'))
            : '—'],
    ];
